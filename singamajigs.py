...
class _Globals:
    # enum the possibility types
    POSSTYPES = {
        'NEXT': 0, # play an existing jig
        'NEW': 1   # add a new one
    }

    # enum the costs as well
    COSTS = {
        'SAME': 0, # same jig plays multiple notes in a row
        'DIFF': 1, # different jig already in set plays the next note
        'ADD':  3, # add a new jig to the set
...
class _Singamajig:
    ...
    def __init__(self, songIndex):
        ...
        self.skin = random.choice(self.songData['skins'])
    
    def getNextIndex(self, prevIndex):
        return prevIndex % len(self.notes)
...
class _Possibility:
    ...
    def advance(self, jigIndex):
        ...
        state.noteIndex = state.singamajig.getNextIndex()
        if jigIndex == self.currentJig:
            self.cost += _globals.COSTS['SAME']
        else:
            self.cost += _globals.COSTS['DIFF']
    ...
    def getAdvanceableJigIndexes(self, note):
        ...
        for state in self.states:
            nextNote = state
            if state.singamajig.getNote(state.noteIndex + 1) == note:
...
def singamajigs(melody):
    ...
    allSingamajigs = []
    for songIndex in range(0, len(_globals.singamajig_data)):
